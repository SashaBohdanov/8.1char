...
int CountCommas(char* s) {
    ...
    {
        pos = t - s; // pos = позиція символа ',' в рядку
        k++;
        if (k == 3)
            return pos; // Повертаємо позицію третьої коми
        pos++; // Переходимо на наступний символ після ','
    }
...
char* Change(char* s) {
    ...
    *t = 0;
    ...
    {
        pos2 = p - s; // pos2 = позиція символа ',' в рядку
        strncat(t, s + pos1, pos2 - pos1); // Додаємо все, що перед комою
        strcat(t, "**");
        pos1 = pos2 + 1; // Переходимо на наступний символ після ','
    }
